...
public class Fruits {
    ...
    private static List<Entity> fillLabyrinth(Loader loader, TexturedModel TexturedModel) {
        ...
        float fruitSize = 5.0f;
        float initialOffset = 3.0f;

        Map map = new Map();

        Vector3f[] fruitPositions = map.generateMapObjects(terrainHeight, cubeSpacing, fruitSize, initialOffset, '*').toArray(new Vector3f[0]);
                /*{
    ...
    public void resetPosition(Loader loader, TexturedModel texturedModel) {
        fruitsPos.clear();
        fruitsPos.addAll(fillLabyrinth(loader, texturedModel));
    }
}